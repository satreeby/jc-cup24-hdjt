...
def process_data(master, env_list, bot_dict, top_dict, dielectric_list, boundpoly, T, id):
    ...
    if T == 'PLATE2L' or T == 'PLATE3L':
        ...
        if 'c2' not in env_names:      # 只要c2不在，rEnv一定不在
            ...
            for e in env_list:
                if e.name == 'c2':
                    c2_lx = (e.topLeft[0]+e.botLeft[0])/2
                    c2_rx = (e.topRight[0]+e.botRight[0])/2
                    info.cond_sep.append(round(c2_lx-master_rx, 6))
            if 'rEnv' not in env_names:
                info.cond_sep.append(4.5)
                # info.cond_sep.append(round(boundpoly.points[1][0]-c2_rx, 6))
            else:
                for e in env_list:
                    if e.name == 'rEnv':
                        rEnv_lx = (e.topLeft[0]+e.botLeft[0])/2
                        rEnv_rx = (e.topRight[0]+e.botRight[0])/2
                        info.cond_sep.append(round(rEnv_lx-c2_rx, 6))
                        # info.cond_sep.append(round(boundpoly.points[1][0]-rEnv_rx, 6))
    elif T == 'STACK3L':
        if id%2 == 1:
            # 下层
            env_names = []
    ...
    # metal_y需要分type
    if T == 'PLATE2L' or T == 'PLATE3L':
        info.metal_y = round((master.topLeft[1]+master.botLeft[1])/2, 6)
    elif T == 'STACK3L':
        info.metal_y = round((master.topLeft[1]+master.botLeft[1])/2, 6)
        for e in env_list:
            if e.name == 'c2':
                info.metal_y_2 = round((e.topLeft[1]+e.botLeft[1])/2, 6)
                break
...
def parser(pattern,     
    ...
    if pattern == 'PLATE2L':
        ...
        for i in range(0, input_num):
            ...
            info = process_data(master, env_list, bot_dict, top_dict, dielectric_list, boundpoly, 'PLATE2L', i)
            feature_matrix.append(output(info, i, 'PLATE2L'))
...
if __name__ == '__main__':
    feature_matrix = parser(pattern='PLATE2L', metal='1', pattern_path='./Cases')
    ...
    file = open("./parser_work/parser_output/PLATE2L/file1.txt", "w+")
    content = str(array)
    file.write(content)
    file.close()
