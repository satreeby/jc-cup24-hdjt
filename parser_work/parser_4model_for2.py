...
def process_data(master, env_list, bot_dict, top_dict, dielectric_list, boundpoly, T, id):
    ...
    if T == 'PLATE3L':
        if id%2==1:
            info.top_H = round(top_dict['topleft'].points[0][1]-master.topLeft[1], 6)
        else:
            info.top_H = round(top_dict['topleft'].points[0][1]-bot_dict['botleft'].points[3][1], 6)
    ...
    if T == 'PLATE2L':
        ...
        if model_type == 0:
            ...
            if id % 2 == 1:
                ...
                re_info.append(info.cond_thickness/info.top_H)
                for value in info.env_top_width.values():
                    re_info.append(value)
                for value in info.env_bot_width.values():
                    re_info.append(value)
        ...
        if generation:
            if pattern == 'PLATE2L':
                for i in range(0, input_num):
                    ...
                    feature_list, space = output(info, i+1, pattern, T)
                    feature_matrix[T].append(feature_list)
            ...
            if pattern == 'PLATE2L':
                ...
                for i in range(0, input_num):
                    ...
                    feature_list, space = output(info, i+1, 'PLATE2L', T)
                    feature_matrix[T].append(feature_list)
                    model_index[T].append(i)
...
if __name__ == '__main__':
    ...
    '''output'''
    #如果是读入所有文件，返回feature_matrix,model_index和space_index。
    # 对于前两个，其中包括了：
    ...
    # 对于space_index，其包括了4个list:



    #如果是读入单个文件，返回该文件的特征（一维list）以及所属的类型
    #注：使用feature_matrix[i]和model_index[i]和space_index[i]的时候需要检查其是否为空
    
    feature_matrix, model_index= parser_features(pattern='PLATE3L', metal='1', 
                                         pattern_path='E:/FDUFiles/DasanX/College_IC_Competition/pythonProject/creat dataset/PLATE3L_single_expand/1/input_files',
    ...
    with open('./test_parser.txt', 'w') as f:
        print(feature_matrix, file=f)
        print("", file=f)
        print(model_index, file=f)
