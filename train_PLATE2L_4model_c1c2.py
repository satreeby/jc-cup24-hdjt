...
val_index = np.delete(np.arange(n_total), train_index)
...
inputs_metal1=torch.tensor(feature_matrix_metal1[1],dtype=torch.float32)
...
inputs_metal2=torch.tensor(feature_matrix_metal2[1],dtype=torch.float32)
...
inputs_metal3=torch.tensor(feature_matrix_metal3[1],dtype=torch.float32)
...
inputs_metal456=torch.tensor(feature_matrix_metal456[1],dtype=torch.float32)
feature_matrix_extentions, _ = parser_4model_for1.parser_features(pattern='PLATE2L', metal='1', 
                                         pattern_path='./Cases/PLATE2L/PLATE2L_4model_c1c2/PLATE2L_random_file',
                                         generation=True, input_num=5832,
                                         single_file=False, 
                                         single_file_path1='',
                                         single_file_path2=''
                                         )
inputs_metal_extentions=torch.tensor(feature_matrix_extentions[1],dtype=torch.float32)

inputs_train = inputs_metal_extentions[train_index,:]
inputs_val = inputs_metal_extentions[val_index,:]
...
# inputs_train=torch.cat((inputs_metal1, inputs_metal2, inputs_metal3, inputs_metal456))[train_index,:].cuda()
...
labels_metal_extentions=cap_utils.load_data('./Cases/PLATE2L/PLATE2L_4model_c1c2/final_output.txt', 0, 5832)
labels_metal1=labels_metal1[model_index_metal1[1],:]
labels_metal2=labels_metal2[model_index_metal2[1],:]
labels_metal3=labels_metal3[model_index_metal3[1],:]
labels_metal456=labels_metal456[model_index_metal456[1],:]

labels_train=labels_metal_extentions[train_index,:]
labels_train=labels_train[:,[1, 3]].cuda()
labels_val=labels_metal_extentions[val_index,:]
labels_val=labels_val[:,[1, 3]]
# labels_train=torch.cat((labels_metal1, labels_metal2, labels_metal3,labels_metal456))[train_index,0:2].cuda()
# labels_val=torch.cat((labels_metal1, labels_metal2, labels_metal3))[:,0:2]
...
# 初始化模型、损失函数和优化器
batch_size=5832
model = CpMLP.CpMLP(inchans=17, hidden1=3096, hidden2=1024, hidden3=512, hidden4=128, hidden5=128, outchans=2)
for param in model.parameters():
    torch.nn.init.normal_(param, mean=0, std=0.01)
# state_dict = torch.load('./model/PLATE2L/saved/model_PLATE2L_4model_c1c2.pt')
...
# 保存模型
torch.save(model.state_dict(), './model/PLATE2L/model_PLATE2L_4model_c1c2_2.pt') """
