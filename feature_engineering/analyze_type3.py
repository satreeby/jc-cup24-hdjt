'''
假设：
1. master都是梯形，且梯形上下底都是平的(即认为所有点中y最大的两个点构成上边，y最小的两个点构成下边)
2. 环境导体是矩形或者梯形，没有其它可能
3. 梯形的宽度用上、下边的均值表示，高度用最大高度范围表示
4. master（c1）总是在下层，环境导体d2总是在上层
'''
...
class Env:
    def __init__(self, name, num, points):
        ...
        # 判断形状(仅用上下底的长度判断)
        self.shape = ''
        if abs((self.topRight[0]-self.topLeft[0])-(self.botRight[0]-self.botLeft[0]))<0.001:
            self.shape = 'rectangle'
        else:
            self.shape = 'trapezoid'
...
def process_data(master, env_list, bot_list, dielectric_list, boundpoly):
    ...
    for i in range(2, 2 + len(env_list)):
        sep_distance.append(
            (sep_x[str(i) + 'lt'] - sep_x[str(i - 1) + 'rt'] + sep_x[str(i) + 'lb'] - sep_x[str(i - 1) + 'rb']) / 2)
    sep_distance.append(
        boundpoly.points[1][0] - (sep_x[str(1 + len(env_list)) + 'rt'] + sep_x[str(1 + len(env_list)) + 'rb']) / 2)
    ...
    layer2_ybot = master.points[0][1]
    master_y_list = []
    for i in range(len(master.points)):
        master_y_list.append(master.points[i][1])
    layer2_yup = max(master_y_list)
    layer1_ybot = 0
    for e in env_list:
        if e.name == 'd2':
            layer1_ybot = e.points[0][1]
    print("导体层间高度差：", layer1_ybot - layer2_yup)
    print("下层导体到bottom高度差：", layer2_ybot - bot_list[0].points[3][1])
    ...
    info.cond_to_cond = round(layer1_ybot - layer2_yup, 3)
    info.cond_to_layer = round(layer2_ybot - bot_list[0].points[3][1], 3)
...
def output_info(INFO_ALL, input_num):
    ...
    with open('output3.txt', 'w') as f:
        ...
        for i in range(input_num):
            print('input ' + str(i+1) + ':', file=f)
            print("    最左边导体到左边界的水平距离：", INFO_ALL.cond_sepL[i][0], file=f)
            for j in range(1, len(INFO_ALL.cond_sepL[i]) - 1):
                print("    第" + str(j) + "个导体和第" + str(j + 1) + "个导体间的水平距离：", INFO_ALL.cond_sepL[i][j], file=f)
            print("    最右边导体到右边界的水平距离：", INFO_ALL.cond_sepL[i][-1], file=f)
        print(" ", file=f)
...
for i in range(0, input_num):
    file_path = '../data/type3_data/BEM_INPUT_'+str(i+1)+'_43924.txt'
