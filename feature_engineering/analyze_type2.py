...
def process_data(master, env_list, bot_list, dielectric_list, boundpoly):
    ...
    # 计算上层导体到下层导体间的高度差，和下层导体到bottom的高度差
    # 用master左下角的点y坐标-下层导体d2的最大y坐标
    # 用下层导体d2左下角点y坐标-bottom顶层坐标
    # d2总是用4个点描述
    layer2_yup = 0
    layer2_ybot = 0
    for e in env_list:
        if e.name == 'd2':
            layer2_ybot = e.points[0][1]
            layer2_yup = e.points[3][1]
    print("导体层间高度差：", master.points[0][1] - layer2_yup)
    ...
    info.cond_to_cond = round(master.points[0][1] - layer2_yup, 3)
...
for i in range(0, input_num):
    file_path = '../data/type2_data/BEM_INPUT_'+str(i+1)+'_43817.txt'
