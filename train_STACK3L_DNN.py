...
inputs_metal1=torch.tensor(parser.parser(pattern='STACK3L', metal='12', pattern_path='./Cases'),dtype=torch.float32)
inputs_metal2=torch.tensor(parser.parser(pattern='STACK3L', metal='23', pattern_path='./Cases'),dtype=torch.float32)
inputs_metal3=torch.tensor(parser.parser(pattern='STACK3L', metal='13', pattern_path='./Cases'),dtype=torch.float32)
...
file.close()
file = open("./log/STACK3L/eval_index.txt", "w+")
...
# 初始化模型、损失函数和优化器
batch_size=345
model = CpMLP.CpMLP(inchans=31, hidden1=500, hidden2=300, hidden3=200, hidden4=200, outchans=7)
...
file = open("./log/STACK3L/results.txt.txt", "w+")
