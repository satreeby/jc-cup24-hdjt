...
labels_train = load_data('C:\project\python\HuadaJiutian\labels\labels_type2.txt', 0, 45)
labels_test = load_data('C:\project\python\HuadaJiutian\labels\labels_type2.txt', 45, 48)

# 获取输入数据

inputs=torch.tensor(parser_6_features.parser(type=2, Fpath='./data'),dtype=torch.float32)
inputs_train=inputs[0:45, :]
inputs_test=inputs[45:, :]
...
# 初始化模型、损失函数和优化器
batch_size=45
model = CpMLP.CpMLP(inchans=6, hidden1=1024, hidden2=512, hidden3=256, outchans=4)
