...
from parser_work import parser_coordinates, parser_pixel
...
# 获取输入数据
# metal1数据
inputs_metal1=torch.tensor(parser_pixel.parser(patternPath=".\Cases", pattern='PLATE2L', metal='1', divw=230, divh=200, r=4),dtype=torch.float32).unsqueeze(dim=3).permute(0, 3, 1, 2)

# metal2数据
inputs_metal2=torch.tensor(parser_pixel.parser(patternPath=".\Cases", pattern='PLATE2L', metal='2', divw=230, divh=200, r=4),dtype=torch.float32).unsqueeze(dim=3).permute(0, 3, 1, 2)

# metal3数据
inputs_metal3=torch.tensor(parser_pixel.parser(patternPath=".\Cases", pattern='PLATE2L', metal='3', divw=230, divh=200, r=4),dtype=torch.float32).unsqueeze(dim=3).permute(0, 3, 1, 2)
...
model = CpCNN.CPCNN(vocab_size=230, embedding_dim=512, hidden1=1024, hidden2=512, hidden3=256, hidden4=128, hidden5=64, out_chans=3).cuda()
